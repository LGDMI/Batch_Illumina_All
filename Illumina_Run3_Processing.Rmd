...
brewset <- brewer.pal(5,"Set2")
md.mdsjac <- metadata[rownames(mdsjac$points),]
for(i in 2:nlevels(factor(md.mdsjac$EM))){
  colvect[which(md.mdsjac$EM==levels(factor(md.mdsjac$EM))[i])]<- brewset[i]
}                                             # for loop assigns colors to levels of EM
points(mdsjac,pch=16,col=colvect)
legend("bottomleft",
       legend = levels(factor(md.mdsjac$EM)),
       col=c("blue",brewset[2:nlevels(factor(md.mdsjac$EM))]),
       pch = 16)
...
NMDSjac <- ggplot(data = DataNMDSJac, aes(MDS1, MDS2),shape=as.factor(EM_Conc), color=as.factor(EM))+
  geom_point()+
